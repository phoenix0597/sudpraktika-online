...
- Универсальный контракт следующей партии для ИИ-агента создан в `data/review/_TASK_PARSE_NEXT_BATCH_AGY.md`; рабочая папка партии — `data/parse_batches/2026-06-26-zpp-next/`, стартовый CSV содержит 37 новых кандидатов после дедупликации. Контракт ссылается на `memory/topic-parsing-strategy.md` как decision-support файл.
...
1. Передать выбранному ИИ-агенту задачу `data/review/_TASK_PARSE_NEXT_BATCH_AGY.md` и рабочую папку `data/parse_batches/2026-06-26-zpp-next/`.
2. Затем настроить локальный веб-сервер (`http://localhost/...`) с нормальными абсолютными маршрутами.
3. После этого обсудить CI/CD через GitHub.
4. Затем вернуться к позиционированию/домену.
...
1. **Передать выбранному ИИ-агенту задачу на следующую партию парсинга/структурирования** — `data/review/_TASK_PARSE_NEXT_BATCH_AGY.md`; стартовая точка `data/parse_batches/2026-06-26-zpp-next/batch_state.md`.
...
4. **Обсуждение позиционирования, брендинга и домена:** провести анализ и выбор названия/слогана/домена на основе концепции из `memory/topic-positioning.md`; отдельно решить судьбу `sudpraktika-online.ru` до 2026-07-01.
