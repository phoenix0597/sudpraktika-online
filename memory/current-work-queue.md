...
1. Закоммитить текущий незакоммиченный UI/validation-пакет.
2. Принять решение по дедупликации парсинга: как избегать повторного fetch уже спарсенных судебных актов.
3. Обсудить объёмы и содержимое следующего парсинга: регионы, ситуации, источники/запросы, целевой размер партии.
4. Сформулировать задачу для Agy на парсинг, чтобы агент работал параллельно.
5. Затем настроить локальный веб-сервер (`http://localhost/...`) с нормальными абсолютными маршрутами.
6. После этого обсудить CI/CD через GitHub.
7. Затем вернуться к позиционированию/домену.
...
1. **Обсудить и принять решение по дедупликации парсинга** — как избегать повторного fetch уже спарсенных судебных актов: хранить реестр в БД/файле, сверять по `docid`/URL/хэшу текста, когда проверять найденное дело против локального реестра.
2. **Обсудить объёмы и содержимое следующего парсинга** — регионы, ситуации/кластеры, источники и поисковые запросы, целевой размер партии, лимиты и критерии включения/hold.
3. **Поставить задачу Agy на парсинг** — после решений по дедупликации и объёму партии сформулировать короткое задание для параллельной работы агента.
4. **Настроить локальный веб-сервер для разработки** — смотреть `site_prototype/` через `http://localhost/...`, вернуть/проверять абсолютные маршруты как перед деплоем и не подгонять ссылки под `file://`.
