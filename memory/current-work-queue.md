...
**Проверить первый SSG-прототип как пользовательские страницы**: открыть `site_prototype/` через локальный сервер, оценить структуру, карточки дел, фильтры, читаемость, fallback для пустых сумм/таймлайна и достаточность данных для финального шаблона.
...
1. **Проверить прототип как пользовательскую страницу** — структура, карточки дел, фильтры, читаемость, fallback для пустых `claimed_total`/`timeline`.
2. **Обсудить и принять решение по дедупликации парсинга** — как избегать повторного fetch уже спарсенных судебных актов: хранить реестр в БД/файле, сверять по `docid`/URL/хэшу текста, когда проверять найденное дело против локального реестра.
3. **Донастроить `timeline` и `amounts` точечно** только если прототип покажет реальные дыры в шаблоне.
4. **Обсуждение позиционирования и брендинга:** провести многосторонний анализ и выбор названия/слогана/домена на основе концепции из `memory/topic-positioning.md` перед финальными шаблонами страниц.
