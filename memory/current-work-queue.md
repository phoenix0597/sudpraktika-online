...
1. Настроить локальный веб-сервер (`http://localhost/...`) с нормальными абсолютными маршрутами.
2. Активные многосторонние обсуждения — см. отдельный раздел «Активные обсуждения» ниже.
...
1. **Настроить локальный веб-сервер для разработки** — смотреть `site_prototype/` через `http://localhost/...`, вернуть/проверять абсолютные маршруты как перед деплоем и не подгонять ссылки под `file://`.
2. **Обсудить CI/CD через GitHub** — что нужно: GitHub-репозиторий/ветки, GitHub Actions, цель деплоя, секреты, preview/prod окружения, порядок проверки перед публикацией.
3. **Обсудить доменную стратегию** — особенно `sudpraktika-online.ru`, срок продления до 2026-07-01; решить, сохраняем ли как широкий домен под сервис судебной практики.
4. **Обсудить и принять решение по дедупликации парсинга** — как избегать повторного fetch уже спарсенных судебных актов: хранить реестр в БД/файле, сверять по `docid`/URL/хэшу текста, когда проверять найденное дело против локального реестра.
5. **Донастроить `timeline` и `amounts` точечно** только если прототип покажет реальные дыры в шаблоне.
6. **Обсуждение позиционирования и брендинга:** провести многосторонний анализ и выбор названия/слогана/домена на основе концепции из `memory/topic-positioning.md` перед финальными шаблонами страниц.
