...
При определении приоритетов очередной партии учитывай `memory/topic-content-fill-plan.md`: он задаёт целевые пороги наполнения по ситуациям, регионам, судам и инстанциям. `memory/topic-parsing-strategy.md` задаёт правила принятия решений внутри партии. Не дублируй эти правила в контракте; при расхождении ориентируйся на актуальные topic-файлы.
...
Важно: `new_candidates.csv` — это не сырой список поиска, а уже очищенный список. Перед fetch в нём не должно быть `docid`, которые уже есть в реестре, raw/structured-артефактах или другом активном batch-каталоге. Если найдено пересечение — не считай такие строки новыми, зафиксируй `skipped_existing`/`duplicate_in_other_batch` в отчёте и уменьши число новых кандидатов.
...
Принятые дополнительные ситуации ЗоЗПП см. в `memory/topic-content-fill-plan.md` в разделе «Принятые новые ситуации ЗоЗПП». Коды из этого раздела уже закреплены в `data/reference/zpp_enum_dictionary.json`; используй их как обычные валидные `dispute_type_code`.

Если найдено дело, похожее на новую ситуацию, но соответствующего кода ещё нет в `data/reference/zpp_enum_dictionary.json`:

- не используй кандидатный код в `taxonomy.dispute_type_code`;
- не форсируй классификацию через неподходящий существующий код;
- не создавай новую страницу-ситуацию и не включай дело в основной индекс;
- зафиксируй URL/docid, предполагаемый код, правовую основу и причину несоответствия текущему словарю в отчёте партии;
- если таких дел несколько, вынеси в `batch_state.md` blocker/next_action: требуется решение пользователя по расширению словаря.

Изменение словаря, переразметка кандидатов и генерация страниц по новой ситуации выполняются только отдельным подтверждённым шагом после решения пользователя.

Инстанционный фокус: первичный массив текущего раздела — суды общей юрисдикции. После набора достаточной низовой практики по ситуации полезно добирать по той же категории апелляционные и кассационные акты, ВС РФ и, когда применимо, КС РФ. Арбитражные суды и ВАС РФ не смешивать с текущим ЗоПП-разделом без отдельного решения пользователя.
...
site:sudact.ru/regular/doc/ "статья 25" "защите прав потребителей" "обмен товара"
site:sudact.ru/regular/doc/ "статья 16" "защите прав потребителей" "навязанная услуга"
site:sudact.ru/regular/doc/ "статья 14" "защите прав потребителей" "вред имуществу"
site:sudact.ru/regular/doc/ "статья 35" "защите прав потребителей" "утрата вещи"
```
...
- Использовать внешний LLM API или скрипты, которые его вызывают (например, DeepSeek API, `scripts/complete_structures_deepseek.py`, OpenAI API, Claude API, Gemini API, YandexGPT API), если пользователь в этой конкретной задаче явно не разрешил такой API. Запрет действует для любого агента и не зависит от его имени; разрешение из прошлой партии или для другого агента не переносится автоматически.
- Делать git commit.
